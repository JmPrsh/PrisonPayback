...
public class RightJoystick : MonoBehaviour, IDragHandler, IPointerUpHandler, IPointerDownHandler
{
    ...
    [Tooltip("Sets the amount distance of the joystick handle (knob) stays away from the center of this joystick. If the joystick handle doesn't look or feel right you can change this value. Must be a whole number. Default valueCurrentis 4.")]
        ...
        if (RectTransformUtility.ScreenPointToLocalPointInRectangle(bgImage.rectTransform, ped.position, ped.pressEventCamera, out localPoint))
        {
            ...
            */
            ...
            /*
                ...
                so that dragging to a corner of the image (for example the bottom left corner) will give a valueCurrentof x,y (-1,-1) which is not correct because the 
                the circular part of the image does not actually reach that far

                the solution to this (further down this script) will be to normalize the vector (remove it's magnitude) so that its valueCurrentof x or y never increases past 1
            */

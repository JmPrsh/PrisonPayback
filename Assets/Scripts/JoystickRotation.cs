...
public class JoystickRotation : MonoBehaviour
{
		...
		public int fingerID;				// ID of finger touching this Joystick.
		...
		private void GetPosition ()
		{
				foreach (Touch touch in Input.touches) {
						...
						if (fingerID >= 0 && fingerID < Input.touchCount) {
								if (Input.GetTouch (fingerID).position.x < Screen.width - Screen.width / 3 && Input.GetTouch (fingerID).position.y < Screen.height / 3 && Input.GetTouch (fingerID).phase == TouchPhase.Began) {
										reset = false;
		...
		private void GetConstraints ()
		{
				...
				if (origin.x > Screen.width - Screen.width / 3) {
						origin.x = Screen.width - Screen.width / 3;
				}
				if (origin.y > Screen.width - Screen.height / 3) {
		...
		private void Awake ()
		{
				gameObject.transform.localScale = new Vector3 (0.2f, 0.3f, 0);
				gameObject.transform.position = new Vector3 (0.35f, 0, 999);
				...
				joystick.texture = joystick2D;
				joystick.color = inactiveColor;
				backOBJ = new GameObject ("VJR-Joystick Rotate");
				backOBJ.transform.localScale = new Vector3 (0, 0, 0);
				backOBJ.transform.position = new Vector3 (0.35f, 0, 0);
				...
				tapTimer = 0;
				length = 50;
				position = new Vector2 ((Screen.width - Screen.width / 3) / 2, (Screen.height / 3) / 2);
				origin = position;
				gotPosition = false;
				EnableJoystick ();
				enable = true;
						...
						if (Input.touchCount > 0 && fingerID > -1 && fingerID < Input.touchCount && gotPosition == true) {
								foreach (Touch touch in Input.touches) {
										if (touch.fingerId == fingerID) {
												...
												if (Input.GetTouch (fingerID).position.x > (Screen.width - Screen.width / 3) + background.pixelInset.width
												...
												Debug.Log ("Joystick Axis:: " + VJRnormals); //<-- Delete this line | (X,Y), from -1.0 to +1.0 | Use this valueCurrent"VJRnormals" in your scripts.
										}
								}
						}
